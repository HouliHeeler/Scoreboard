...
function Scoreboards({scores, stats, colour, colourAway}) {
    ...
    return (
        scores.map((item, idx) => {
            const homeColour = Object.values(nbaColours.find(el => el[((item.teams.home.name).split(" ").join(""))]))[0][0]
            const awayColour = Object.values(nbaColours.find(el => el[((item.teams.visitors.name).split(" ").join(""))]))[0][0]
            ...
            return (
                <div className="scorecard" key={idx}>
                    <div className='boxscore' 
                         ...
                         style={{background: `linear-gradient(135deg, ${homeColour}, white, ${awayColour})`}} >
